...
> สถานะ: **✅ ย้ายเสร็จครบ Phase 0–7** — dc app ถูกลบ, แอปเป็น Vite+React (`web/`) เต็มตัว, exe build/รันผ่าน, ออฟไลน์; branch `feat/vite-migration` (main ยังเป็น dc เดิมจน merge)
...
- [x] **4b · PrintView** (`*`) — scan bar (Fg + ค้น), grid `<table>` (Sku/Plu/Cat/Desc/Supplier/Price/Qty stepper + ลบแถว), media A4/roll + roll steppers, **live preview**, `PoModal`, `ScanResultsModal`, พิมพ์
  - **render core (ใช้ซ้ำใน 4c):** store editor slice (elements/labelW/H/bg/media + `bootTemplates`) + data/PO slice + `ElementBox`/`BarcodeCanvas`/`QrBox`/`LabelPreview` (วาดผ่าน `barcode.ts` ใน `useEffect`)
  - ✅ **print แบบ data-URL (Phase 5 ดึงมารวม):** `lib/printDoc.ts` pre-render บาร์โค้ด/QR → PNG data-URL + ฝัง `@font-face` จาก parent → popup เป็น HTML ล้วน **ไม่พึ่ง `/vendor/` ไม่มี script**
  - **เช็คแล้ว (headless):** สแกนรหัสจริง → เจอหลายตัว → modal → เพิ่มทั้งหมด (dedup) → grid 2 แถว, preview วาดบาร์โค้ด 2 ดวง, พิมพ์ → doc 514KB มี 4 รูป data-URL, `hasVendor:false` `hasScript:false` `@font-face:true`, 0 error
- [x] **4c · DesignView** (`*`) — `Canvas` (drag/resize/zoom/pan/guides/8 handles, snap + Alt free, Space pan) + `Inspector` (TRANSFORM/ALIGN/CONTENT ครบ 6 ชนิด/color/binding/image upload/bg) + `DesignSidebar` (ELEMENTS/TEMPLATES/SAVED/SIZE) + shortcuts (Arrow±0.5/Shift±2, Del, Esc, Ctrl+D) + `ConfirmDupModal`
  - store editor mutations: select/add/patch/dup/delete/bg/size/preset/zoom + templates (load/save/overwrite/open/delete)
  - ใช้ render core จาก 4b (`ElementBox`/`BarcodeCanvas`/`QrBox`)
  - **เช็คแล้ว (headless):** add text (x4,y12 centered) → drag (x8,y15) → resize SE handle (w34,h7.5) → align-left (x0) → size A4 (210×297) → save (toast + เข้า SAVED list → `templates.json` จริง), 0 error · ลบ template ทดสอบออกแล้ว

> **Phase 4 เสร็จครบ 3 หน้า** (Settings/Print/Design) — ทุกหน้า verify จริงเทียบแอปเดิม

### Phase 5 — Print window — ✅ เสร็จ (ทำใน 4b + verify ที่นี่)
- [x] วิธี render ในโลก bundled = **data-URL** (`lib/printDoc.ts`) — ไม่มี `/vendor/`, ไม่มี script ในป๊อปอัป
- [x] เพิ่ม `font-family` IBM Plex ที่ body ของป๊อปอัป (กัน fallback)
- **เช็คแล้ว (headless, เสิร์ฟจาก dist):** เปิดป๊อปอัปจริง → 4 label, บาร์โค้ดเป็นรูป data-URL 4 รูป, Thai "ขนมลูกชุบ" render ด้วย IBM Plex, ออฟไลน์ (blockedExternal: []) — screenshot ผ่าน

### Phase 6 — Cutover + แพ็ก exe — ✅ เสร็จ
- [x] `server.js` static root → `web/dist` (`WEB_DIR`) + SPA fallback → `index.html`; `/api/*` คงเดิม
- [x] `package.json` `pkg.assets` → `web/dist/**/*`; scripts: `build:web`, `dev:web`, `build:exe`
- [x] `scripts/build-exe.js`: เพิ่ม step `vite build` (`npm --prefix web run build`, ใช้ `shell:true` กัน EINVAL บน .cmd) ก่อน pkg
- **เช็คแล้ว:** `npm run build:exe` → `dist/Geniuz_Barcode.exe` (101MB) · รัน exe → เสิร์ฟ bundled app (`id=root` + `/assets/*`) ที่ :8080, `/api/health` 200, ออฟไลน์
- ⚠️ ยังไม่ได้ทดสอบ exe ต่อ SQL จริง (ต้องวาง `.env` ข้าง exe ใน `dist/`) — ทำตอน deploy

> **หมายเหตุ:** main ยังเป็นแอป dc เดิม; ของเก่า (`.dc.html`/`support.js`/`vendor/`) ยังอยู่บน branch — ลบใน Phase 7

### Phase 7 — Cleanup + docs — ✅ เสร็จ
- [x] ลบ `LabelDesigner.dc.html`, `support.js`, `vendor/` (40 ไฟล์), `scripts/fetch-vendor.js`, root `index.html` (tag `pre-vite-cutover` กันไว้)
- [x] เขียน [Architecture.md](Architecture.md) ใหม่ (โครง Vite) + ADR **D9** ใน [Decisions.md](Decisions.md) (ยกเลิก D1)
- [x] rewrite [Gotchas.md](Gotchas.md) (ตัด gotcha dc-specific, เพิ่ม dev-2-process/data-URL print/build-exe) + note ใน [Roadmap.md](Roadmap.md)
- [x] อัปเดต `deploy/client/README.md` (offline) + `run.bat` (build web ครั้งแรก แล้ว node server.js)
- **เช็คแล้ว:** `node --check server.js` OK, `npm run build` (web) ผ่าน, vitest 22/22, ไม่มี ref ไป dc app เหลือในโค้ด
