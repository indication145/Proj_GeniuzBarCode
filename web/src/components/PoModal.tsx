  ...
  return (
    <div style={overlay} onClick={closePo}>
      <div style={sheet} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--c-efedea)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600 }}>พิมพ์จากใบสั่งซื้อ · Purchase Order</div>
            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>เลือกใบสั่งซื้อเพื่อโหลดรายการสินค้ามาพิมพ์</div>
          </div>
          <button onClick={closePo} style={{ width: 30, height: 30, border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface)', cursor: 'pointer' }}>
            ✕
          </button>
        </div>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--c-efedea)', display: 'flex', flexWrap: isMobile ? 'wrap' : 'nowrap', gap: 8 }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 14 }}>
          {poBusy && <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 24 }}>กำลังโหลด…</div>}
          {!poBusy && poList.length === 0 && <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 24, fontSize: 13 }}>ไม่พบใบสั่งซื้อ</div>}
          {poList.map((r, i) => {
